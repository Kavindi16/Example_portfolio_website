...
import { BiBriefcase, BiMailSend, BiRightArrowAlt, BiHome, BiCode } from 'react-icons/bi'
    ...
    <div className='md:flex-[1]'>
      <div className='min-h-screen top-0 hidden md:sticky md:flex items-center justify-center bg-teal-600 text-white'>
        <ul className='flex flex-col gap-10'>
          {["Home",'Experience','Project','Contact'].
          map((item) => (
            <li key={item} className='flex items-center justify-start cursor-pointer font-medium transition-all duration-200 group sm:text-lg md:text-xl xl:text-3xl'>
              ...
              -translate-x-5 opacity-0 transform transition-all duration-200 group-hover:opacity-100 group-hover:translate-x-0
              '/>

              < a href={`#${item}`}
              className='transition-all duration-200
              hover-translate-x-3'>{item}
              </a>
            </li>
          ))}
      ...
      <div className='fixed left-0 right-0 top-0 z-0 flex justify-evenly bg-teal-600 p-5 text-white md:hidden'>
